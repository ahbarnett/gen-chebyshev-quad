...
author: Alex H. Barnett, Center for Computational Mathematics, Flatiron Institute

A scratch repo for codes for GCQ. Such codes create a custom 1D quadrature scheme (nodes and weights) that integrate a given family of functions to a user-prescribed relative tolerance $\epsilon$. The algorithm is, in words:
 ...
 2. Fill the matrix $A$ which has each column one function in the family evaluated on the fine nodes, multiplied by the square-root of the weights $v$.
 ...
 4. Find a subset of rows that span the row space of $U$, for instance doing CPQR on $U^T$ and keeping the row indices given by the pivots. The output nodes $x$ are the fine nodes with these skeleton indices.
 5. The output weights $w$ solve the usual "Vandermonde transpose" linear system imposing that the functions in the unweighted columns of $U$ have the correct integrals. (Specifically, the matrix is the transpose of the skeletonized $U$, column-weighted by the square-roots of $v$, while the right-hand side is the row sums of the square-root row-weighted transpose of the original $U$ matrix.)

Examining the code is probably clearer at this point. Extended precision may be needed for Step 5 for more tricky function families, but for benign singularities working precision is enough if you don't care about full accuracy. If the size of the family is not large (<100), then the cost is usually dominated by the number of fine nodes (in which it is linear). The cost is usually less than one second on a laptop, in double precision. GCQ is the initial step in generalized Gaussian quadrature (GGQ; see first reference below), which can reduce the number of nodes by up to a factor of 2, in 1D. However, GCQ (unlike the full GGQ) is simple, fast (unless you have a family of thousands of functions) and automatic.

### Example

In Julia, our implementation (applying to real-valued functions) is called ``genchebquad``. Here we use it to build a custom 12-digit accurate quadrature on (-1,1) for smooth functions plus functions are smooth times a logarithmic singularity at ``x0=0.6`` (inside the integration interval):

```julia
a, b = -1.0, 1.0                                                    # interval
x0 = 0.6                                                            # log singularity
fs = x -> reduce(vcat, x^k .* [1, log(abs(x - x0))] for k = 0:20)   # family of 42 funcs
x, w = genchebquad(fs, a, b, 1e-12)                                 # get nodes, weights
println("num nodes = ", length(x))
f = x -> sin(1 + 3x)                                                # smooth test func
fp = x -> 3cos(1 + 3x)                                              # fp is deriv of f
println("smooth error = ", abs(sum(w .* fp.(x)) - f(b) + f(a)))     # test the FTC
f = x -> sin(3 * (x - x0)) * log(abs(x - x0))                       # log-singular test
fp = x -> 3cos(3(x - x0)) * log(abs(x - x0)) + 3sinc(3(x - x0) / pi)
println("smooth.log error = ", abs(sum(w .* fp.(x)) - f(b) + f(a)))
```
...
Note the clustering around the singularity location ``x0 = 0.6``. See the docstring in ``julia/src`` for another example.
