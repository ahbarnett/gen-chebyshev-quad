function [p, info] = rowpivots_chebfun(f,n)
...
%
% [p,info] = rowpivots_chebfun(f,n) generates n new nodes per chebfun panel,
%  trying for more accuracy, but the problem is f has already lost accuracy.
%
...
if nargin<2, n=[]; end   % empty indicates original chebfun nodes used in x,w

N = size(f,2);
[x,w] = getquad_chebfun(f,n);   % extract implied quadrature scheme (m nodes)
if isempty(n)
  ...
  ff = f(x);         % re-eval chebfun at all nodes (problem is f lost acc :()
end
